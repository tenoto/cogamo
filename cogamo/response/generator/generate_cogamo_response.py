...
print("=====================================")
print("generate_cogamo_response.py")
...
# 4: Initial particle position x (cm) 
# 5: Initial particle position y (cm)
# event_list_file = '%s/response_CsI_coated.txt' % os.getenv('COGAMO_RESPONSE_DATA_PATH')

print("--- reading data file... take time...")
with open(EVENT_LIST_FILE) as f:
	firstline = f.readline().rstrip()
num_of_columns = len(firstline.split())
...
if num_of_columns == 3:
	...
	df = pd.read_csv(EVENT_LIST_FILE,
		names=["event number","initial energy","deposit energy","init x","init y"],
		delim_whitespace=True)
...
for i in range(nbin_initial_energy):
	if hist1d_init[i] > 0:
		...
		if sum(hist2d_count[i]>0.0) == 0:
			list_normalized_count.append(hist2d_count[i])
		else:
			print("Error: something wrong at the normalization.")
			exit()
